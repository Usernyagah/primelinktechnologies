...
import { productsApi, servicesApi } from '@/lib/db';
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <TabsList className="bg-secondary/40 border border-border">
              <TabsTrigger value="products" className="data-[state=active]:bg-accent data-[state=active]:text-accent-foreground">
            ...
            <Dialog open={isDialogOpen} onOpenChange={(open) => !open && handleCloseDialog()}>
              ...
              <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  <DialogTitle>{editingItem ? 'Edit' : 'Add New'} {activeTab === 'products' ? 'Product' : 'Service'}</DialogTitle>
                </DialogHeader>
                {activeTab === 'products' ? (
                  <ProductForm 
                  ...
                  <ServiceForm 
                    initialValues={editingItem} 
                    onSubmit={(values) => editingItem ? updateService.mutate(values) : createService.mutate(values)}
                ...
                <tbody className="divide-y divide-border">
                  {loadingServices ? (
                    ...
                    <tr key={s.id} className="hover:bg-secondary/10 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <img src={s.image} className="w-10 h-10 rounded object-cover" alt="" />
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          <Button variant="ghost" size="icon" className="text-destructive" onClick={() => deleteService.mutate(s.id)}>
                            <Trash2 className="w-4 h-4" />
                          </Button>
                        </div>
                      </td>
